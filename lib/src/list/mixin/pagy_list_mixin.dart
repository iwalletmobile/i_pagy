...
mixin _PagyMixin<T> on State<Pagy<T>> {
  late ScrollController
      scrollController; // Scroll controller for tracking scroll position
  ValueNotifier<bool> showLoadingNotifier = ValueNotifier<bool>(
      false); // Notifier to control loading indicator visibility
  ...
  void didUpdateWidget(Pagy<T> oldWidget) {
    ...
    if (widget.items.isNotNull &&
        widget.items!.isNotEmpty &&
        (oldWidget.items?.isNotEmpty ?? false) &&
